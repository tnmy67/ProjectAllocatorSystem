...
@Component({
  selector: 'app-allocator-list',
  templateUrl: './allocator-list.component.html',
  styleUrls: ['./allocator-list.component.css']
})
export class AllocatorListComponent implements OnInit {
  employees: Employee[] = [];
  ...
  constructor(private authService: AuthService, private allocatorService: AllocatorService, private cdr: ChangeDetectorRef, private route: Router) { }
  ...
  totalEmployeesCount(search?: string) {
    this.allocatorService.getEmployeesCount(search)
      .subscribe({
  ...
  loadAllEmployees(): void {
    // this.loading = true;
    this.allocatorService.getAllEmployees().subscribe({
  ...
  loadPaginatedEmployees(search?: string, sortBy?: string) {
    // this.loading = true;
    this.allocatorService.getAllEmployeesByPagination(this.currentPage, this.pageSize, this.sortOrder, search, sortBy)
      .subscribe({
        next: (response: ApiResponse<Employee[]>) => {
          if (response.success) {
